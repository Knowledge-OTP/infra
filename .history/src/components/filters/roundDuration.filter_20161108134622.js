    ...
    function Filter($filter) {
        ...
        return function FilterFilter(time) {
            ...
            if (time <= ONE_MIN_IN_MILLISECONDS) {
                ...
                numOfSec = remainedSec ? Math.round(remainedSec /1000): '';
                filteredTime = numOfMin + ' min ' + numOfSec + ' sec';
            } else {
                remainedMin = time%ONE_HOUR_IN_MILLISECONDS;
                remainedSec = remainedMin%ONE_MIN_IN_MILLISECONDS;
                numOfHours = Math.floor(time/ONE_HOUR_IN_MILLISECONDS);
                numOfMin = Math.floor(remainedMin/ONE_MIN_IN_MILLISECONDS);
                numOfSec = Math.round(remainedSec/1000);
                filteredTime = numOfHours + 'h ' + numOfMin + ' min ' + numOfSec + ' sec';
            }
            return filteredTime;
        }
    }
