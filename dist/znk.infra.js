...
/**
 * evaluates content , then it appended it to the DOM , and finally it compiles it with scope which
 * was created out of the directive scope. attrs- compile-drv: expression which be evaluated and
 * then appended to the dom. bind-once: angular expression which evaluated by the scope , if it
 * true then the watcher will be killed after the first time content was added to the dom
 */
            ...
            return {
                ...
                link: function (scope, element, attrs) {
                    ...
                    var watchDestroyer = scope.$watch(attrs.compile, function (newVal) {
                        ...
                        var _htmlStrRegex = /^<(\w+)( .*|)>(.|\n)*(<\/\1>|)$/;
                        /**
                         * check if html string , if true create jq lite element of it and append
                         * with animation otherwise just append to the dom
                         */
...
angular.module('znk.infra').run(['$templateCache', function($templateCache) {
  $templateCache.put("components/general/templates/timerDrv.html",
    ...
    "</div>\n" +
    "");
  $templateCache.put("components/general/templates/videoPlayer.html",
    "<video ng-show=\"isAvail\"controls preload = \"none\">\n" +
    "          <source  type = \"video/mp4\"/>\n" +
    " </video>\n" +
    "<div class=\"video_not_available\" ng-hide=\"isAvail\"></div>\n" +
    "\n" +
    "");
